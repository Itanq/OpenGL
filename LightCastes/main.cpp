...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 光的属性设置
		Uint lightAmbientLoc = glGetUniformLocation(containerShader.Program, "light.ambient");
		Uint lightDiffuseLoc = glGetUniformLocation(containerShader.Program, "light.diffuse");
		Uint lightSpecularLoc = glGetUniformLocation(containerShader.Program, "light.specular");
		Uint lightPositionLoc = glGetUniformLocation(containerShader.Program, "light.position");

		// 平行光只需要知道光的方向即可
		//glUniform3f(glGetUniformLocation(containerShader.Program, "light.direction"), -0.2f,-1.0f,-0.3f);
		...
		glUniform3f(glGetUniformLocation(containerShader.Program, "light.direction"), camera.Front.x, camera.Front.y, camera.Front.z);
		glUniform1f(glGetUniformLocation(containerShader.Program, "light.cutOff"), glm::cos(glm::radians(12.5f)));


		// 点光 衰减公式的因子
		glUniform1f(glGetUniformLocation(containerShader.Program, "light.constant"), 1.0f);
		glUniform1f(glGetUniformLocation(containerShader.Program, "light.linear"), 0.09f);
		glUniform1f(glGetUniformLocation(containerShader.Program, "light.quadratic"), 0.032f);
